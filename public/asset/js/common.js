...
//所有切换样式
if ($('.js-ajax-changeStatus').length) {
    Wind.use('artDialog', function () {
        $('.js-ajax-changeStatus').on('click', function (e) {
            e.preventDefault();
            var $_this = this,
                $this = $($_this),
                href = $this.data('href'),
 ...
 * @param callback
 */
function open_moveto_dialog(url, title,input_name,options, callback){
    var ids = [];
